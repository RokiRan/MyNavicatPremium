...
| 导出 | 表级：CSV / JSON / SQL(INSERT 语句，含 DROP+CREATE，可在任意库重放）；库级：mysqldump 风格整库 SQL 转储（含视图定义，不导视图数据），原子写文件 |
| 数据库管理 | 连接右键「新建数据库…」：字符集/排序规则从服务器读取并联动过滤，非法输入客户端拦截 |
...
- **导出**：选中表标签页 → 工具栏「导出」，或侧栏表名右键「导出…」；库名右键「导出数据库…」导出整库 SQL 转储（可选只导结构）
- **新建数据库**：连接名右键「新建数据库…」，选字符集/排序规则（默认 utf8mb4）
...
Tests/MyNavicatCoreTests/   # 25 个集成测试（真实连接 MySQL）+ 5 个存储单测
```
...
测试会创建/销毁 `mynavicat_test_a`、`mynavicat_test_b`、`mynavicat_test_created` 三个临时库，覆盖：连接、元数据、结构、分页、DML、中文/NULL/BLOB/BIT 往返、三种格式导出、SQL 重放、跨库迁移、生成列、同库防护、行编辑/新增/删除、无主键拒绝、建库字符集与非法输入拦截、整库转储重放。存储单测覆盖：密码不落盘、旧版明文迁移、Keychain 优先、删除连接清理密码。
